...
NSString * StringWithTrailingSlash(NSString* str);

#define RREQ RouteRequest
#define RRES RouteResponse
#define ROUTES RouteRequest *request, RouteResponse *response
...
//	By default this is NULL and routes are processed on CocoaHTTPServer's connection queue.
//	You can specify a queue to process routes on, such as dispatch_get_main_queue() to process all routes on the main thread.
- (dispatch_queue_t)routeQueue;
- (void)setRouteQueue:(dispatch_queue_t)queue;

@property NSMutableDictionary* mimeTypes;

-      (void)setMIMEType:(NSString*)type forExtension:(NSString*)ext;
- (NSString*)mimeTypeForPath:(NSString*)path;

// Convenience methods. Yes I know, this is Cocoa and we don't use convenience methods because typing lengthy primitives over and over and over again
// is elegant with the beauty and the poetry. These are just, you know, here.
